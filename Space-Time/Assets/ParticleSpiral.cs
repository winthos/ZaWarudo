...
public class ParticleSpiral : MonoBehaviour 
{ 
	// -- particleEffect --
...
public class ParticleSpiral : MonoBehaviour 
{
  ...
  [SerializeField]
  int particlesperArm = 200;
  
  [SerializeField]
  float particleDist = 0.05f;
  
  [SerializeField]
  float turnDist = 0.5f;
  
  [SerializeField]
  float verticalTurnDist =0.0f;
  ...
  [SerializeField]
  float fadePoint = 0.0f;

  [SerializeField]
  float sizeAtPoint = 0.0f;
  ...
  {
    Transform effectObject = (Transform)Instantiate(particleEffect, transform.position, transform.rotation);
    effectObject.parent = gameObject.transform;
    ...
    emitter.Emit(Arms * particlesperArm);
    
    Particle[] p = emitter.particles;
    
    float armRoom = (Mathf.PI * 2) / Arms;
    
    for (int i = 0; i < Arms; i++)
    {
      float radius = 0;
      float theta = 0;
      float armRot = i * armRoom;
      
      for (int j = 0; j < particlesperArm; j++)
      {
        int pIndex = i * particlesperArm + j;
        
        radius = originOffset + turnDist * theta;
        
        Vector3 pos = effectObject.localPosition;
        
        pos.x = pos.x + radius * Mathf.Cos(theta);
        pos.z = pos.z + radius * Mathf.Sin(theta);
        pos.x = pos.x * Mathf.Cos(armRot) + pos.z * Mathf.Sin(armRot);
        pos.z = pos.x * -Mathf.Sin(armRot) + pos.z * Mathf.Cos(armRot);

        pos.y = pos.y + j * verticalTurnDist;
        
        if (emitter.useWorldSpace)
          pos = transform.TransformPoint(pos);
        p[pIndex].position = pos;
        
        theta += particleDist;
        
        p[pIndex].energy = p[pIndex].energy - j * fadePoint;
        p[pIndex].size = p[pIndex].size - j * sizeAtPoint;
      }
    }
  }
	...
	void Update () 
  {
    transform.Rotate(transform.up * Time.deltaTime * -turnSpeed, Space.World);
	}
  void LateUpdate()
  {
    float timelastSpawn = Time.time - lastSpawn;
    if (timelastSpawn >= spawnRate && particleCount < spawns)
    {
      SpawnParticleEffect();
      lastSpawn = Time.time;
      particleCount++;
